...
def spectrum_to_see(picture):
    ...
    # Handle zero values
    small_value = 1e-10  # You can adjust this value as needed, it should be small enough not to affect the overall spectrum significantly
    magnitude_spectrum[magnitude_spectrum == 0] = small_value
    # Apply logarithmic transformation
    magnitude_spectrum = 20 * np.log(magnitude_spectrum)
    magnitude_spectrum[np.isinf(magnitude_spectrum)] = 0  # Replace -inf values with 0
    magnitude_spectrum[np.isnan(magnitude_spectrum)] = 0  # Replace nan values with 0
