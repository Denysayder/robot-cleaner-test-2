...
def process_video_perspective(input_file):
    ...
    output = cv.VideoWriter(output_file, fourcc, fps, (300, 300))
    ...
    while True:
        ...
        points = np.array([[width * multiplier_width, height * multiplier_height],
                           [width * (1 - multiplier_width), height * multiplier_height],
                           [width * (1 - multiplier_width), height * (1 - multiplier_height)],
                           [width * multiplier_width, height * (1 - multiplier_height)]], dtype=np.float32)

        m = getTransform(points)

        # Apply perspective transformation
        dst = cv.warpPerspective(np.float32(img_ori), m, (300, 300))
...
def perform_realtime_perspective_transform(frame, width, height):
    ...
    transformed_frame = cv.warpPerspective(frame, m, (300, 300))
